...
class BlockstateResolver {
  ...
  /**
   * Extract all possible property names and values from a blockstate
   * Used for complete state enumeration
   * 
   * @param {string} blockName - Block name (e.g., "oak_slab")
   * @returns {Object|null} Map of property name → Set of possible values
   *   e.g., { type: Set(['bottom', 'top', 'double']), waterlogged: Set(['true', 'false']) }
   */
  extractPropertyDomains(blockName) {
    let normalized = blockName.replace('minecraft:', '');
    ...
    const blockstate = this.blockstates.get(normalized);
    if (!blockstate) return null;
    
    const domains = {};
    
    if (blockstate.variants) {
      // Variants format: extract from variant keys
      for (const propsStr of Object.keys(blockstate.variants)) {
        if (!propsStr) continue; // Empty string = no properties
        
        const props = this._parsePropsString(propsStr);
        for (const [key, value] of Object.entries(props)) {
          if (!domains[key]) {
            domains[key] = new Set();
          }
          domains[key].add(String(value));
        }
      }
    } else if (blockstate.multipart) {
      // Multipart format: extract from all 'when' conditions
      this._extractMultipartDomains(blockstate.multipart, domains);
    }
  ...
  /**
   * Extract property domains from multipart conditions recursively
   * @private
   */
  _extractMultipartDomains(multipart, domains) {
    for (const part of multipart) {
      if (part.when) {
        this._extractConditionDomains(part.when, domains);
      }
    }
  }
  
  /**
   * Extract property domains from a single condition
   * @private
   */
  _extractConditionDomains(when, domains) {
    // Handle OR conditions
    if (when.OR) {
      for (const cond of when.OR) {
        this._extractConditionDomains(cond, domains);
      }
      return;
    }
    ...
    // Extract property values from this condition
    for (const [key, value] of Object.entries(when)) {
      if (!domains[key]) {
        domains[key] = new Set();
      }
      
      // Handle pipe-separated values (e.g., "true|false")
      if (typeof value === 'string' && value.includes('|')) {
  ...
  /**
   * Generate all possible property combinations from domains
   * @param {Object} domains - Property name → Set of values
   * @returns {Array<Object>} Array of property objects
   */
  generateAllCombinations(domains) {
    if (!domains || Object.keys(domains).length === 0) {
      return [{}]; // Single empty state
    }
    
    const keys = Object.keys(domains).sort();
    const values = keys.map(k => Array.from(domains[k]));
